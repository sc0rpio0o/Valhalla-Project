...
void Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32 &duration,Unit* caster,DiminishingLevels Level, int32 limitduration)
{
    ...
    if((GetDiminishingReturnsGroupType(group) == DRTYPE_PLAYER && (GetTypeId() == TYPEID_PLAYER || GetCharmerOrOwner()->GetTypeId() == TYPEID_PLAYER)) || GetDiminishingReturnsGroupType(group) == DRTYPE_ALL)
    {
